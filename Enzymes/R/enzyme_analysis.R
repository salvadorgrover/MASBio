...
ggsave("bg_date_treat.jpeg", plot = sup, width = 80, height = 80, units = "mm", dpi = 300)
...
per.offsite$activity = per.offsite$activity * 1000 
...
onsite.all <- rbind(bg.clean, per.clean) %>% 
  rbind(ppo.clean) %>% 
  ungroup() %>% 
  dplyr::select(-treatment, -mean, -two.sd, -z.score)
...
#BG
#Mine
mine_off_on <- enzyme.on.off %>% 
  subset(land_type == "Mine")

model <- lm(BG ~ crop, data = mine_off_on)
ggqqplot(residuals(model))
shapiro.test(residuals(model))
...
anova <- aov(BG ~ crop, data = mine_off_on)
anova
tukey <- TukeyHSD(anova)
tukey
...
#Ag 
ag_off_on <- enzyme.on.off %>% 
  subset(land_type == "Agriculture")

model <- lm(BG ~ crop, data = ag_off_on)
ggqqplot(residuals(model))
shapiro.test(residuals(model))
bc <- boxcox(model)
(lambda <- bc$x[which.max(bc$y)])
new_model <- lm(((BG^lambda-1)/lambda) ~ crop, data = ag_off_on)
ggqqplot(residuals(new_model))
shapiro.test(residuals(new_model))
ag_off_on$bg_bc <- (ag_off_on$BG^lambda-1)/lambda

ag_off_on %>% 
  ungroup() %>% 
  levene_test(bg_bc ~ crop)

anova <- aov(bg_bc ~ crop, data = ag_off_on)
anova
tukey <- TukeyHSD(anova)
tukey
tuk <- HSD.test(anova, trt = "crop", group = T)
...
kruskal.test(BG ~ crop, data = ag_off_on)
ag_off_on %>% 
  dunn_test(BG ~ crop, 
            p.adjust.method = "BH")
...
model <- lm(PER ~ crop, data = mine_off_on)
ggqqplot(residuals(model))
shapiro.test(residuals(model))
bc <- boxcox(model)
(lambda <- bc$x[which.max(bc$y)])
new_model <- lm(((PER^lambda-1)/lambda) ~ crop, data = mine_off_on)
ggqqplot(residuals(new_model))
shapiro.test(residuals(new_model))
mine_off_on$per_bc <- (mine_off_on$PER^lambda-1)/lambda

mine_off_on %>% 
  ungroup() %>% 
  levene_test(per_bc ~ crop)

anova <- aov(per_bc ~ crop, data = mine_off_on)
anova
tukey <- TukeyHSD(anova)
tukey
tuk <- HSD.test(anova, trt = "crop", group = T)
...
kruskal.test(PER ~ crop, data = mine_off_on)
mine_off_on %>% 
  dunn_test(BG ~ crop, 
            p.adjust.method = "BH")
...
model <- lm(PER ~ crop, data = ag_off_on)
ggqqplot(residuals(model))
shapiro.test(residuals(model))
bc <- boxcox(model)
(lambda <- bc$x[which.max(bc$y)])
...
anova <- aov(PER ~ crop, data = ag_off_on)
anova
tukey <- TukeyHSD(anova)
tukey
tuk <- HSD.test(anova, trt = "crop", group = T)
...
p1 <- ggplot(data = dt, aes(x = date, y = mean, fill = crop)) +
  #geom_bar(position = "dodge", stat = "identity") +
  geom_bar_pattern(position = "dodge", stat = "identity",
                   aes(pattern = crop),
                   fill            = 'white', 
                   colour          = 'black',
                   pattern_spacing = 0.023)  +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.2,
                position=position_dodge(.9)) +
  facet_wrap(land_type ~ .) + 
  theme_classic() +
...
p2 <- ggplot(data = dt, aes(x = land_type, y = mean, fill = land_type)) +
...
p3 <- ggplot(data = dt, aes(x = date, y = mean, fill = "white")) +
  geom_bar(stat = "identity",
           width = 0.6,
           fill = "white",
           color = "black")  +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.2,
                position=position_dodge(.9)) +
  theme_classic() +
  labs(x = "", y = PER~Activity~"(nmol"~h^-1~g^-1~")") +
  geom_text(aes(label = cld, y = mean + se), position=position_dodge(width=0.9), vjust = -0.5, size = 3) +
...
ggsave("bg_per_panel.jpeg", plot = fig2, width = 180, height = 127, units = "mm", dpi = 300)
ggsave("bg_date_treat.jpeg", plot = fig2, width = 80, height = 80, units = "mm", dpi = 300)

#dev.new(width=180, height = 180, unit="mm")
