...
sup <- ggplot(data = dt, aes(x = date, y = mean, fill = crop)) +
  #geom_bar(position = "dodge", stat = "identity") +
  ...
  facet_wrap(land_type ~ .) + 
  theme_classic() +
  labs(x = "", y = "POXC (mg/kg of soil)", color = "Site") +
...
ggsave("poxc_date_site_crop.jpeg", plot = sup, width = 180, height = 83, units = "mm", dpi = 300)
...
poxc_new <- poxc.all %>% 
  dplyr::select(c("site","poxc", "date", "land_type", "crop")) %>% 
...
#Mine
mine_off_on <- poxc_off_on %>% 
  subset(land_type == "Mine")

model <- lm(poxc ~ crop, data = mine_off_on)
ggqqplot(residuals(model))
shapiro.test(residuals(model))
...
mine_off_on %>% 
  ungroup() %>% 
  levene_test(poxc ~ crop)

anova <- aov(poxc ~ crop, data = mine_off_on)
anova
tukey <- TukeyHSD(anova)
tukey
...
#Ag 
ag_off_on <- poxc_off_on %>% 
  subset(land_type == "Agriculture")

model <- lm(poxc ~ crop, data = ag_off_on)
ggqqplot(residuals(model))
shapiro.test(residuals(model))
...
anova <- aov(poxc ~ crop, data = ag_off_on)
anova
tukey <- TukeyHSD(anova)
tukey
tuk <- HSD.test(anova, trt = "crop", group = T)
